...
//  1. normalizeWhitespace — safe to apply automatically and silently.
//     Trimming and collapsing double-spaces is never ambiguous; it can
//     only ever be correct.
//  2. findSimilarValue — deliberately NOT automatic. Punctuation and
//     casing differences ("H.O.D Godfrey" vs "HoD Godfrey", "M-Pesa" vs
//     "MPesa") can be genuinely meaningful, not just typos — silently
//     rewriting one into the other risks quietly merging two things that
//     were actually different. This only ever WARNS, and the caller
//     decides what to do with that warning.

/** Trims and collapses any run of internal whitespace (spaces, tabs, newlines) to a single space. Always safe to apply automatically. */
export function normalizeWhitespace(value: string): string {
  return value.trim().replace(/\s+/g, ' ')
}

/** Aggressive form used only for comparison, never for storage or display: lowercase, every non-alphanumeric character stripped. */
function collapseForComparison(value: string): string {
  return value.toLowerCase().replace(/[^a-z0-9]/g, '')
}

/**
 * Checks `value` against a list of existing values for a likely typo-level
 * duplicate — same underlying text once case and punctuation are ignored,
 * but not an exact match. Returns the existing value it most likely
 * duplicates, or null if there's no exact-normalized match. Never returns
 * a match for `value` against itself (an exact match to something already
 * in the list isn't a "near duplicate", it's just the same value).
 */
export function findSimilarValue(value: string, existing: string[]): string | null {
  const normalized = normalizeWhitespace(value)
  const collapsed = collapseForComparison(normalized)
  if (!collapsed) return null
  for (const e of existing) {
    if (e === normalized) return null   // exact match — not a "near duplicate", it's the same thing
    if (collapseForComparison(e) === collapsed) return e
  }
  return null
}
