...
export interface TransactionRecord {
  ...
  recordedAt:     string
  importBatchId:  string    // shared by every row from one bulk import — blank for manually-entered rows. Lets an entire import be found and reversed as a group.
}
...
export interface TargetRecord {
  ...
  region:                string    // which hospital/branch this target applies to. Required for
                                    // Gateway and SalesRep targets (the only types created going
                                    // forward — see docs/TARGETS_MODEL.md). Blank on legacy
                                    // Overall/Product/Region rows that predate this field.
...
// (Product). Drives the Department target roll-up — see
// docs/TARGETS_MODEL.md. A scheme with no entry here isn't counted in
// any department's total until mapped.
export interface SchemeDepartmentMapping {
  gateway:    string
  department: string
}

// ── SCHEME → SALES REP MAP ───────────────────────────────────
// Mirrors SchemeDepartmentMapping exactly, for the same reason: Sales Rep
// is no longer a manually-entered target dimension — its target is
// computed by summing every Payment Scheme target whose scheme is mapped
// to that rep. See docs/TARGETS_MODEL.md.
export interface SchemeSalesRepMapping {
  gateway:  string
  salesRep: string
}
