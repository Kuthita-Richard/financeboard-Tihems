      ...
      <div className="max-w-6xl mx-auto px-6 py-10 grid lg:grid-cols-[220px_1fr] gap-8">
        ...
        <div className="space-y-6">
          ...
          <Section id="targets" icon={Target} title="Targets">
            <p>Only <strong>{settings.gatewayLabel}</strong> targets are entered directly, each scoped to a specific <strong>{settings.regionLabel}</strong> — e.g. &quot;SHA Scheme at Masinga = KSh 1.8M&quot;. Set an <strong>Annual Target</strong> for the year and a <strong>Revised Target</strong> for whenever plans change. A target <strong>carries forward automatically</strong> into every following month until a newer entry for that same {settings.regionLabel.toLowerCase()}/{settings.gatewayLabel.toLowerCase()} pair replaces it — including across a new year — so you only re-enter it when it actually changes. Marking one <strong>Inactive</strong> also carries forward, until reactivated or replaced.</p>
            <p><strong>Overall, {settings.regionLabel}, {settings.productLabel}, and {settings.salesRepLabel} are never entered directly</strong> — they&apos;re computed by summing the relevant {settings.gatewayLabel.toLowerCase()} targets. {settings.productLabel} and {settings.salesRepLabel} classification (which {settings.gatewayLabel.toLowerCase()} counts toward which) is auto-detected from the ledger, with manual override available under <strong>Settings → Classifications</strong>.</p>
