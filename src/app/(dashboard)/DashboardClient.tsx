...
// ── Reusable tooltip ──────────────────────────────────────────
const ChartTip = ({ active, payload, label, sym, hint }: {
      ...
      {payload.map((p,i) => (
        <div key={i} className="flex items-center justify-between gap-4 mb-1">
          ...
          <strong style={{ color:'#0c1a2e' }}>
            {Math.abs(p.value) > 100 ? formatCurrencyCompact(p.value, sym) : `${Number(p.value).toFixed(1)}%`}
          </strong>
        </div>
      ))}
...
export default function DashboardClient({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Gauge + Product variance + Gateway ────────────── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Product variance */}
        <Card title={`Variance % by ${settings.productLabel}`} subtitle="Click bar to drill through">
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={perf.byProduct} margin={{ top:18, right:8, left:-22, bottom:0 }}>
              ...
              <Bar dataKey="amountAchievementPct" name="Achievement %" radius={[3,3,0,0]} cursor="pointer"
                ...
                {perf.byProduct.map((d,i) => <Cell key={i} fill={d.amountAchievementPct >= 100 ? PBI.pos : PBI.neg} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Card>
        ...
        {/* Gateway */}
        <Card title={`${settings.gatewayLabel} Collection`} subtitle="Click bar to drill through">
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={perf.byGateway} layout="vertical" margin={{ top:0, right:50, left:0, bottom:0 }}>
              ...
              <Tooltip content={<ChartTip sym={sym} hint={`Click to see ${settings.gatewayLabel} details`} />} />
              <Bar dataKey="actualAmount" name="Collected" fill={PBI.actual} radius={[0,3,3,0]} cursor="pointer"
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      {/* ── Region actual vs targets ───────────────────────── */}
      <Card title={`Actual vs Targets by ${settings.regionLabel}`} subtitle="Click any bar to drill through">
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={perf.byRegion} margin={{ top:12, right:24, left:12, bottom:0 }} barCategoryGap="30%">
            <CartesianGrid strokeDasharray="3 3" stroke="#e0f2fe" vertical={false} />
            ...
            <Bar dataKey="actualAmount"        name="Actual"         fill={PBI.actual}  radius={[3,3,0,0]} barSize={24}
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              cursor="pointer" onClick={(d:any)=>drillTo('region',d.dimensionValue)} />
            <Bar dataKey="amountRevisedTarget" name="Revised Target" fill={PBI.revised} radius={[3,3,0,0]} barSize={24}
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              cursor="pointer" onClick={(d:any)=>drillTo('region',d.dimensionValue)} opacity={0.8} />
            <Bar dataKey="amountAnnualTarget"  name="Annual Plan"    fill={PBI.target}  radius={[3,3,0,0]} barSize={24}
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              cursor="pointer" onClick={(d:any)=>drillTo('region',d.dimensionValue)} opacity={0.5} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
