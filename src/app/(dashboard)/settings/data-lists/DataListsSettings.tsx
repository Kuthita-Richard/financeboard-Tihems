...
function Chip({ column, item }: { column: Column; item: string }) {
  ...
  const onSave = (force = false) => {
    ...
    startTransition(async () => {
      const res = await editMetadataItemAction(column, item, trimmed, force)
      if (res.success) { toast.success(res.message); setEditing(false); setSimilarTo(null) }
      else if (res.similarTo) { setSimilarTo(res.similarTo) }
  ...
  if (similarTo) {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs" style={{ background: '#fffbeb', color: '#b45309' }}>
        <AlertTriangle size={11} />
        Looks like &quot;{similarTo}&quot;?
        <button type="button" onClick={() => { setEditing(false); setSimilarTo(null); setDraft(item) }} disabled={pending}
          className="font-semibold hover:opacity-70 disabled:opacity-40">Use that</button>
        <button type="button" onClick={() => onSave(true)} disabled={pending}
          className="font-semibold hover:opacity-70 disabled:opacity-40">
          {pending ? <Loader2 size={11} className="animate-spin" /> : 'Rename anyway'}
        </button>
      </span>
    )
  }
...
function ListEditor({ column, label, items }: { column: Column; label: string; items: string[] }) {
  ...
  const onAdd = (force = false) => {
    const trimmed = value.trim()
    if (!trimmed) return
    startTransition(async () => {
      const res = await addMetadataItemAction(column, trimmed, force)
      if (res.success) { toast.success(res.message); setValue(''); setSimilarTo(null) }
      else if (res.similarTo) { setSimilarTo(res.similarTo) }
      else toast.error(res.message)
    })
  }
  ...
  return (
    <div className="space-y-3 rounded-xl border p-4" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
      ...
      <div className="flex gap-2">
        <input
          value={value}
          onChange={e => { setValue(e.target.value); setSimilarTo(null) }}
      ...
      {similarTo && (
        <div className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs" style={{ background: '#fffbeb', color: '#b45309' }}>
          <AlertTriangle size={13} className="shrink-0" />
          <span className="flex-1">This looks very similar to <strong>&quot;{similarTo}&quot;</strong> — likely the same thing typed slightly differently.</span>
          <button type="button" onClick={() => { setValue(''); setSimilarTo(null) }} disabled={pending}
            className="font-semibold hover:opacity-70 disabled:opacity-40 shrink-0">Use existing</button>
          <button type="button" onClick={() => onAdd(true)} disabled={pending}
            className="font-semibold hover:opacity-70 disabled:opacity-40 shrink-0">
            {pending ? <Loader2 size={12} className="animate-spin" /> : 'Add anyway'}
          </button>
        </div>
      )}
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs mt-1" style={{ color: 'var(--muted-fg)' }}>
          Removing a value here only stops it from being pre-registered — if it&apos;s already used in a real transaction, it&apos;ll keep appearing in dropdowns until nothing in the ledger references it. Renaming only updates the registry entry itself; it does not rewrite past transactions or targets that used the old text. Adding or renaming to something very similar to an existing value (spacing, punctuation, capitalization) will ask you to confirm first, to catch typos before they fragment your reporting.
        </p>
      </div>
