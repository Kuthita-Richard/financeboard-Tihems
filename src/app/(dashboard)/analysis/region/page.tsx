...
export default async function RegionAnalysisPage({ searchParams }: { searchParams: Promise<Record<string,string>> }) {
  ...
  const label = settings.regionLabel
  return (
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color:'var(--muted-fg)' }}>
            {month} {year} · {perf.byRegion.length} {label.toLowerCase()}s
          </p>
        </div>
        <PeriodFilter years={years} year={year} month={month} />
      </div>
      <AnalysisChart rows={perf.byRegion} sym={sym} />
      <AnalysisTable rows={perf.byRegion} sym={sym} dimLabel={label} />
    </div>
  )
}
