...
export const metadata: Metadata = { title: 'SalesRep Analysis' }
export default async function SalesRepAnalysisPage({ searchParams }: { searchParams: Promise<Record<string,string>> }) {
  ...
  const label = settings.salesRepLabel
  return (
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color:'var(--muted-fg)' }}>
            {month} {year} · {perf.bySalesRep.length} {label.toLowerCase()}s
          </p>
        </div>
        <PeriodFilter years={years} year={year} month={month} />
      </div>
      <AnalysisChart rows={perf.bySalesRep} sym={sym} />
      <AnalysisTable rows={perf.bySalesRep} sym={sym} dimLabel={label} />
    </div>
  )
}
