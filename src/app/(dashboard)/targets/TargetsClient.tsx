...
const DIMENSION_TYPES = ['Overall', 'Product', 'Gateway', 'Region', 'SalesRep'] as const
...
export default function TargetsClient({ targets, settings, metadata, filter }: Props) {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm<TargetInput>({
    ...
    defaultValues: {
      ...
      dimensionType: 'Overall', dimensionValue: 'Overall',
  ...
  const getDimOptions = (type: string) => {
    ...
    if (type === 'Product')  return metadata.products
    if (type === 'Gateway')  return metadata.gateways
    if (type === 'Region')   return metadata.regions
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color:'var(--muted-fg)' }}>
            Set monthly targets per dimension. Every month must have an explicit row.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Dimension Value" error={errors.dimensionValue?.message}>
                {dimType === 'Overall'
                  ? <input type="text" value="Overall" readOnly className={iCls} style={{ ...iStyle, opacity:0.6 }} {...register('dimensionValue')} />
            ...
            <table className="w-full text-xs" style={{ minWidth:900 }}>
              <thead>
                <tr style={{ background:'#eff6ff', borderBottom:'1px solid #bfdbfe' }}>
                  {['Year','Month','Dimension','Value',`Amt Annual Plan`,`Amt Revised`,`Count Plan`,`Count Revised`,'Active','Notes','Actions'].map(h => (
