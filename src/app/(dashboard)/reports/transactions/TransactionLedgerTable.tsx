...
export default function TransactionLedgerTable({ rows, settings, metadata, sym, total }: Props) {
  ...
  const handleDelete = async (row: TransactionRecord) => {
    ...
    // The confirm() dialog above can show the real name/amount — that's
    // ephemeral, client-side, seen only by the admin doing the delete. What
    // gets sent here becomes a permanent Audit sheet row, so it deliberately
    // excludes the three encrypted fields (name/department/amount) to avoid
    // duplicating them in plaintext outside the Ledger.
    const res = await deleteTransactionAction(row.rowIndex, `${row.id} | ${row.date} | ${row.gateway}`)
    setDeletingId(null)
...
function EditTransactionModal({ row, settings, metadata, onClose }: {
  ...
  const onSave = async () => {
    ...
    const res = await updateTransactionAction(
      row.rowIndex,
      { recordedBy: row.recordedBy, recordedByEmail: row.recordedByEmail, recordedAt: row.recordedAt, importBatchId: row.importBatchId },
      form
    )
    setSaving(false)
