...
export default async function ReportProductPage({ searchParams }: { searchParams: Promise<Record<string,string>> }) {
  ...
  return (
    <div className="space-y-5">
      <ReportHeader settings={settings} title={`${settings.productLabel} Performance Report`} />
      <div className="flex items-start justify-between flex-wrap gap-3 no-print">
        <div>
          <h2 className="text-xl font-bold" style={{ color:"var(--fg)" }}>{settings.productLabel} Performance Report</h2>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ borderColor:"#bfdbfe" }}>
        <table className="w-full text-xs"><thead>
          <tr style={{ background:"#eff6ff", borderBottom:"1px solid #bfdbfe" }}>
            {[settings.productLabel,`Actual (${sym})`,`Annual Plan`,`Revised Target`,"vs Revised","vs Annual Plan","Customers","Cust.Target","Active","Status"].map(h => (
          ...
          <tbody>{perf.byProduct.map((row,i) => {
